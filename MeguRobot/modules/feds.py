...
def new_fed(update: Update, context: CallbackContext):
    ...
    if not fednam == "":
        ...
        update.effective_message.reply_text(
            "*Ha logrado crear una nueva federación!*"
            "\nNombre: `{}`"
            "\nID: `{}`"
        ...
        try:
            ...
            LOGGER.warning("Cannot send a message to GLOBAL_LOGS")
    else:
        update.effective_message.reply_text("Escriba el nombre de la federación")
...
def del_fed(update: Update, context: CallbackContext):
    ...
    update.effective_message.reply_text(
        "Seguro que quiere eliminar su federación? Esto no se puede revertir, perderá toda su lista de prohibiciones y '{}' se perderá permanentemente.".format(
            getinfo["fname"]
        ),
        reply_markup=InlineKeyboardMarkup(
            [
                ...
                [
                    InlineKeyboardButton(
                        text="❌ Cancelar ❌", callback_data="rmfed_cancel"
                    )
                ],
            ]
        ),
...
def leave_fed(update: Update, context: CallbackContext):
    ...
    if getuser in "creator" or user.id in SUDO_USERS:
        if sql.chat_leave_fed(chat.id) is True:
            ...
            send_message(
                update.effective_message,
                "Este grupo ha dejado la federación {}!".format(fed_info["fname"]),
            )
        else:
            update.effective_message.reply_text(
...
def user_demote_fed(update: Update, context: CallbackContext):
    ...
    if is_user_fed_owner(fed_id, user.id):
        ...
        if res is True:
            update.effective_message.reply_text(
                "Degradado de administrador de la federación!"
            )
        else:
...
def fed_info(update: Update, context: CallbackContext):
    ...
    text += "\nN° de grupos en esta federación: <code>{}</code>".format(len(getfchat))
...
def fed_ban(update: Update, context: CallbackContext):
    ...
    if is_user_fed_admin(fed_id, user_id) is True:
        message.reply_text(
            "Él es un administrador de la federación, no puedo banearlo de la federación."
        )
        return

    if user_id == OWNER_ID:
        message.reply_text(
            "El propietario del  bot no puede ser baneado de una federación!"
        )
        return

    if int(user_id) in SUDO_USERS:
        message.reply_text("Los dragones no pueden ser baneados de la federación!")
        return

    if int(user_id) in FROG_USERS:
        message.reply_text("Los tigres no pueden ser baneados de la federación!")
        return

    if int(user_id) in WHITELIST_USERS:
        message.reply_text("Los lobos no pueden ser baneados de la federación!")
        return
    ...
    if fban:
        ...
        if not temp:
            message.reply_text(
                "No se pudo actualizar el motivo de ban de la federación!"
            )
            return
        x = sql.fban_user(
            fed_id,
            ...
            try:
                ...
                if excp.message in FBAN_ERRORS:
                    try:
                        ...
                        LOGGER.info(
                            "El chat {} ha abandonado la federación {} porque me echaron".format(
                                fedschat, info["fname"]
                            )
                        )
                        continue
                    ...
                    LOGGER.warning(
                        "No se puedo banear de la federación en {} porque: {}".format(
                            chat, excp.message
                        )
                    )
                    ...
                    try:
                        ...
                        if excp.message in FBAN_ERRORS:
                            try:
                                ...
                                LOGGER.info(
                                    "El chat {} anuló la federación {} porque me echaron".format(
                                        fedschat, info["fname"]
                                    )
                                )
                                continue
...
def unfban(update: Update, context: CallbackContext):
    ...
    message.reply_text(
        "Daré otra oportunidad a {} en esta federación".format(user_chat.first_name)
    )
    ...
    bot.send_message(
        ...
        "\n<b>Administrador de la federación:</b> {}"
        "\n<b>Usuario:</b> {}"
        "\n<b>ID de Usuario:</b> <code>{}</code>".format(
            info["fname"],
            mention_html(user.id, user.first_name),
            user_target,
            fban_user_id,
    ...
    if args:
        ...
        for chat in chat_list:
            ...
            try:
                ...
                failed += 1
                LOGGER.warning(
                    "No se pudo enviar la transmisión a {}".format(str(chat))
                )
...
def fed_ban_list(update: Update, context: CallbackContext):
    ...
    if len(getfban) == 0:
        update.effective_message.reply_text(
            "La lista de baneados de la federación de {} está vacía".format(
                info["fname"]
            ),
            parse_mode=ParseMode.HTML,
        )
        ...
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                ...
                update.effective_message.reply_document(
                    ...
                    caption="Total {} usuarios baneados por la Federación {}.".format(
    ...
    for users in getfban:
        ...
        if getuserinfo is False:
            text = "No hay usuarios baneados de la federación {}".format(info["fname"])
            break
    ...
    try:
        ...
        with BytesIO(str.encode(cleantext)) as output:
            ...
            update.effective_message.reply_document(
                ...
                caption="La siguiente es una lista de usuarios que actualmente están prohibidos en la Federación {}.".format(
                    info["fname"]
                ),
            )
            ...
            with BytesIO() as file:
                ...
                for x in splitting:
                    ...
                    if int(import_userid) in SUDO_USERS:
                        failed += 1
                        continue
                    if int(import_userid) in FROG_USERS:
                        failed += 1
                        continue
...
def fed_stat_user(update: Update, context: CallbackContext):
    ...
    if user_id:
        ...
        if not fedinfo:
            send_message(
                update.effective_message, "Federación {} no encontrada!".format(fed_id)
            )
            return
...
def set_fed_log(update: Update, context: CallbackContext):
    ...
    if args:
        ...
        send_message(
            update.effective_message, "No ha proporcionado ID de la federación!"
        )
...
def subs_feds(update: Update, context: CallbackContext):
    ...
    if is_user_fed_owner(fed_id, user.id) is False:
        send_message(
            update.effective_message,
            "Solo el propietario de la federación puede hacer esto",
        )
        return
    ...
    if args:
        ...
        if subfed:
            send_message(
                update.effective_message,
                "La federación `{}` se ha suscrito la federación `{}`. Cada vez que haya un Fedban de esa federación, esta federación también prohibirá a ese usuario.".format(
            ...
            if get_fedlog:
                if int(get_fedlog) != int(chat.id):
                    bot.send_message(
                        get_fedlog,
                        "La federación `{}` se ha suscrito a la federación `{}`".format(
                            fedinfo["fname"], getfed["fname"]
                        ),
                        parse_mode="markdown",
                    )
...
def fed_admin_help(update: Update, context: CallbackContext):
 ...
 •`/unban <usuario> <razón>`: Desbanea a un usuario de la federación.
