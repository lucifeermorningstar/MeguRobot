...
async def whatanime(c: Client, m: Message):
    ...
    if not media:
        await m.reply_text("Responde a una foto, GIF o video")
        return
    ...
    if isinstance(json, str):
        ...
        try:
            ...
            await reply.edit_text("Sin resultados")
        else:
            nsfw = match["is_adult"]
            ...
            async def _send_preview():
                ...
                with tempfile.NamedTemporaryFile() as file:
                    ...
                    try:
                        ...
                        await reply.reply_text("No puedo enviar la vista previa :/")
...
<b>Descargando:</b> {format_bytes(current)}
<b>Velocidad de Descarga:</b> {download_speed}/s
<b>Tiempo tomado:</b> {calculate_eta(current, total, start_time)}"""
        if prevtext != text:
