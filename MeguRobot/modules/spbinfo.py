...
async def lookup(client, message):
    ...
    if response == True:
        ...
        stats += f" • **Información de chat**: [Link](t.me/SpamProtectionBot/?start=00_{user.id})\n"
        ...
        stats += f' • **Idioma**: `{a["results"]["language_prediction"]["language"]}`\n'
        ...
        stats += (
            f'**En la lista negra**: `{a["results"]["attributes"]["is_blacklisted"]}`\n'
        )
        if a["results"]["attributes"]["is_blacklisted"] == True:
            ...
            stats += (
                f' • **Bandera**: `{a["results"]["attributes"]["blacklist_flag"]}`\n'
            )
