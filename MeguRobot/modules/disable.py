...
if is_module_loaded(FILENAME):
        ...
        if len(args) >= 1:
            ...
            if disable_cmd in set(DISABLE_CMDS + DISABLE_OTHER):
                ...
                if conn:
                    ...
                    text = "Se deshabilitó el uso del comando `{}`!".format(disable_cmd)
                send_message(
                    update.effective_message,
                    text,
                ...
                send_message(
                    update.effective_message, "Este comando no se puede deshabilitar"
                )

        else:
            send_message(update.effective_message, "Que quieres que desactive?")
        ...
        if len(args) >= 1:
            ...
            if sql.enable_command(chat.id, enable_cmd):
                if conn:
                    ...
                    text = "Se habilitó el uso del comando `{}`!".format(enable_cmd)
                send_message(
                    update.effective_message,
                    text,
                    parse_mode=ParseMode.MARKDOWN,
    ...
    def list_cmds(update, context):
        if DISABLE_CMDS + DISABLE_OTHER:
            ...
            update.effective_message.reply_text(
                "Comandos que pueden ser deshabilitados:\n{}".format(result),
                parse_mode=ParseMode.MARKDOWN,
            )
        else:
    ...
    def build_curr_disabled(chat_id: Union[str, int]) -> str:
        ...
        if not disabled:
            return "No hay comandos desactivados!"
    ...
    def __stats__():
        return "{} items deshabilitados, en {} chats.".format(
