...
def clearall(update: Update, context: CallbackContext):
    ...
    if member.status != "creator" and user.id not in SUDO_USERS:
            ...
            [
                [
                    InlineKeyboardButton(
                        text="Eliminar Notas", callback_data="notes_rmall"
                    )
                ],
        ...
        update.effective_message.reply_text(
            f"Estás seguro de que deseas borrar *TODAS* las notas en {chat.title}? Esta acción no se puede deshacer.",
    ...
    if query.data == "notes_rmall":
        if member.status == "creator" or query.from_user.id in SUDO_USERS:
            ...
            try:
                ...
                message.edit_text("Notas borradas.")
            except BadRequest:
                return
...
def list_notes(update: Update, context: CallbackContext):
    ...
    if not note_list:
        try:
            ...
            update.effective_message.reply_text(
                "No hay notas en este chat!", quote=False
            )
...
def __chat_settings__(chat_id, user_id):
    ...
    return f"Hay `{len(notes)}` notas en este chat."
