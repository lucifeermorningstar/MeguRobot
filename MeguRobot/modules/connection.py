...
def allow_connections(update, context) -> str:
    ...
    if chat.type != chat.PRIVATE:
        ...
        send_message(
            update.effective_message, "Este comando es solo para grupos. No en privado!"
        )
        ...
        if args and len(args) >= 1:
            ...
            if (isadmin) or (isallow and ismember) or (user.id in SUDO_USERS):
                ...
                if connection_status:
                    ...
                    send_message(
                        update.effective_message,
                        "Conectado exitosamente a *{}*. \nUsa /helpconnect para ver los comandos disponibles.".format(
                            chat_name
                        ),
                        parse_mode=ParseMode.MARKDOWN,
                    )
                ...
                send_message(
                    update.effective_message, "No se permite la conexión a este chat!"
                )
        else:
...
def disconnect_chat(update, context):
    ...
    if update.effective_chat.type == "private":
        ...
        send_message(
            update.effective_message, "Este comando solo está disponible en PM."
        )
...
def help_connect_chat(update, context):
    ...
    if update.effective_message.chat.type != "private":
        send_message(
            update.effective_message,
            "Hazme un privado con ese comando para obtener ayuda.",
        )
        return
    else:
    ...
    if connect_match:
        ...
        if (isadmin) or (isallow and ismember) or (user.id in SUDO_USERS):
            ...
            context.bot.answer_callback_query(
                query.id, "No se permite la conexión a este chat!", show_alert=True
            )
...
Esto le permite conectarse a la base de datos de un chat y agregar cosas sin que los comandos aparezcan en el chat! Por razones obvias, debe ser administrador para agregar cosas, pero cualquier miembro del grupo puede ver sus datos.
 ...
 •`/connect`: Se conecta al chat (se puede hacer en un grupo mediante `/connect` o `/connect <chat id>` en privado)
