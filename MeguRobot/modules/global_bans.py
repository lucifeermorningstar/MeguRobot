...
def gban(update: Update, context: CallbackContext):
    ...
    if sql.is_user_gbanned(user_id):
        ...
        if old_reason:
            message.reply_text(
                ...
                "<code>{}</code>\n"
                "Lo he actualizado con tu nueva razón!".format(html.escape(old_reason)),
    ...
    if GLOBAL_LOGS:
        try:
            log = bot.send_message(GLOBAL_LOGS, log_message, parse_mode=ParseMode.HTML)
        except BadRequest as excp:
...
def __chat_settings__(chat_id, user_id):
    return (
        f"Este chat está aplicando *baneos  globales*: `{sql.does_chat_gban(chat_id)}`."
    )
...
""".format(
    SUPPORT_CHAT, parse_mode=ParseMode.MARKDOWN, disable_web_page_preview=False
)
