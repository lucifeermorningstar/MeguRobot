...
import telegram
from MeguRobot import BAN_STICKER, FROG_USERS, WHITELIST_USERS, dispatcher
...
from MeguRobot.modules.helper_funcs.string_handling import split_quotes
from MeguRobot.modules.log_channel import loggable
from MeguRobot.modules.sql import warns_sql as sql
...
def add_warn_filter(update: Update, context: CallbackContext):
    ...
    update.effective_message.reply_text(
        f"Controlador de advertencia agregado para '{keyword}'!"
    )
    ...
    if args:
        if args[0].isdigit():
            if int(args[0]) < 3:
                ...
                msg.reply_text(
                    "Se actualizó el límite de advertencias a{}".format(args[0])
                )
                return (
                    f"<b>{html.escape(chat.title)}:</b>\n"
