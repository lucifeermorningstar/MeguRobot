...
import MeguRobot.modules.sql.blacklist_sql as sql
...
from MeguRobot.modules.helper_funcs.regex_helper import (
    infinite_loop_check,
    regex_searcher,
)
...
def blacklist(update: Update, context: CallbackContext):
    ...
    if update_chat_title == message_chat_title:
        base_blacklist_string = (
            "<b>Palabras incluidas actualmente en la lista negra</b>:\n"
        )
    else:
    ...
    if len(words) > 1:
        ...
        for trigger in to_blacklist:
            try:
                ...
                msg.reply_text(
                    f"No se pudo agregar la expresión regular, error: {exce}"
                )
                return
        ...
        if len(to_blacklist) == 1:
            msg.reply_text(
                f"Se agregaron <code>{html.escape(to_blacklist[0])}</code> a la lista negra!",
                parse_mode=ParseMode.HTML,
            )

        else:
            msg.reply_text(
                f"Se agregaron <code>{len(to_blacklist)}</code> palabras a la lista negra.",
                parse_mode=ParseMode.HTML,
            )
...
def unblacklist(update: Update, context: CallbackContext):
    ...
    if len(words) > 1:
        ...
        if len(to_unblacklist) == 1:
            if successful:
                msg.reply_text(
                    f"<code>{html.escape(to_unblacklist[0])}</code> removida de la lista negra!",
                    parse_mode=ParseMode.HTML,
                )
            else:
                msg.reply_text("Esta palabra no esta en la lista negra...")

        elif successful == len(to_unblacklist):
            msg.reply_text(
                f"Se eliminaron <code>{successful}</code> palabras de la lista negra.",
                parse_mode=ParseMode.HTML,
            )

        elif not successful:
            msg.reply_text(
                "Ninguna de estas palabras está, por lo que no se eliminaron.",
                parse_mode=ParseMode.HTML,
            )
...
def __stats__():
    return "{} palabras en lista negra, en {} chats.".format(
