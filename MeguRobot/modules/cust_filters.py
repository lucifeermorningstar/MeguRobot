...
def stop_filter(update, context):
    ...
    for keyword in chat_filters:
        if keyword == args[1]:
            ...
            send_message(
                update.effective_message,
                "Ok, dejaré de responder a ese trigger en *{}*.".format(chat_name),
                parse_mode=telegram.ParseMode.MARKDOWN,
            )
            raise DispatcherHandlerStop
...
def addnew_filter(update, chat_id, keyword, text, file_type, file_id, buttons):
    ...
    if len(totalfilt) >= 1000:  # Idk why i made this like function....
        msg.reply_text("Este grupo ha alcanzado su límite máximo de 150 triggers.")
        return False
    else:
...
def __stats__():
    return "{} triggers, en {} chats.".format(sql.num_filters(), sql.num_chats())
...
RMALLFILTER_HANDLER = CommandHandler(
    "rmalltriggers", rmall_filters, filters=Filters.chat_type.groups
)
RMALLFILTER_CALLBACK = CallbackQueryHandler(rmall_callback, pattern=r"filters_.*")
LIST_HANDLER = DisableAbleCommandHandler("triggers", list_handlers, admin_ok=True)
