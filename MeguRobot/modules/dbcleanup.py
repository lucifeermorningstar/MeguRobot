...
import MeguRobot.modules.sql.global_bans_sql as gban_sql
import MeguRobot.modules.sql.users_sql as user_sql
from MeguRobot import DEV_USERS, OWNER_ID, dispatcher
...
def dbcleanup(update: Update, context: CallbackContext):
    ...
    msg.reply_text("Obteniendo un recuento de baneados globalmente no válidos...")
    invalid_gban_count = get_invalid_gban(update, context)

    reply = f"Total de chats no válidos - {invalid_chat_count}\n"
    reply += (
        f"Total de usuarios de baneados globalmente no válidos - {invalid_gban_count}"
    )
